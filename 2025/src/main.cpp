...
void go(double dist, double power){
  ...
  double proportionalControllerConstant = 0.125;//abs(100/dist)
  int time = 0;
  while (true){
    ...
    if (abs(left_average) < 1.5 and abs(right_average) < 1.5){
      time ++;
...
void turn(double target, double power){
  int time = 0;
  double proportionalControllerConstant = 0.6;
  while(true){
...
void test2(){
  ...
  middleIntake.spin(forward,-50,percent);
  wait(50,msec);
  frontIntake.spin(forward,-80,percent);
  middleIntake.spin(forward,80,percent);
  wait(1.5,sec);
  go(-2.5,10);
  wait(0.01,sec);
  go(-46,100);
...
void usercontrol(void) {
  ...
  while (1) {
    ...
    if (Controller1.ButtonDown.pressing() == true and reversepress == false){
